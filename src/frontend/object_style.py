from PyQt6.QtWidgets import QVBoxLayout,QWidget,QLabel,QScrollArea,QFileDialog,QHBoxLayout,QSizePolicy,QComboBox
...
class ObjectStyle(QWidget):
    ...
    def control_area(self, layout):
        ...
        self.status_label = self.gui_helper.create_label("Edit all objects, select the output format, and press 'Create Assert' (use '.png' for best transparency).",parent=self.control_widget)
        self.status_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.status_label.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        self.control_layout.addWidget(self.status_label)
        

        self.slider_list = ['Pixel Art','Dithered Art','Game Boy','Cel Shading','Simple Animation','Anime']
        ...
        self.output_format_dropbox = QComboBox(parent=self.control_widget)
        self.output_format_dropbox.setStyleSheet("""QComboBox {padding: 2px;border: 1px solid #cccccc;border-radius: 2px;} QComboBox:hover {border: 1px solid #a0a0a0;} QComboBox::drop-down {subcontrol-origin: padding;subcontrol-position: top right;width: 20px;}""")
        output_formats = ['.png','.jpeg','.webp','.jpg']
        for formats in output_formats:
            self.output_format_dropbox.addItem(formats)
        self.output_format_dropbox.currentTextChanged.connect(lambda selected_format : setattr(self,'output_format',selected_format))
        self.control_layout.addWidget(self.output_format_dropbox,alignment = Qt.AlignmentFlag.AlignCenter)

        self.edit_confirm_btn  = self.gui_helper.create_button('Create Assert',False,button_width=130,button_height=30,parent=self.control_widget)
    ...
    def set_slider_change_fun(self,slider_name,val,label):
        ...
        if self.current_image_name:
            ...
            if hasattr(self, "worker_thread_addstyle") and self.worker_thread_addstyle.isRunning():
                self.worker_thread_addstyle.wait()

            self.worker_thread_addstyle = WorkerThread(self.add_styles_to_image)
            self.worker_thread_addstyle.finished.connect(self.update_display_image)
            self.worker_thread_addstyle.start()


    def add_styles_to_image(self,progress_signal):

        self.edit_manager.apply_edits_to_display(self.current_image_name,'Assert','add_image_styles',self.slider_value_dict[self.current_image_name])
    ...
    def confirm_edited_images(self,progress_signal):
        ...
        for i,(image_name,image) in enumerate(self.extracted_object_refined.items()):
            ...
            if self.edit_manager.command_stack['Undo'].get(image_name):
                ...
                edited_image = EditOptions().add_image_styles(current_image,edits)
    ...
    def close_window(self,final_obj_dict):
        ...
        folder_path = QFileDialog.getExistingDirectory(parent=self,caption="Select Output Folder",directory="")
        if folder_path:
            for image_name,image in final_obj_dict.items():
                file_path = Path(folder_path) / f"{image_name}{self.output_format}"
                cv2.imwrite(str(file_path), image)
            no_of_objects = len(final_obj_dict)
            self.close()
            self.setAttribute(Qt.WidgetAttribute.WA_DeleteOnClose, True)
            self.feedback_viewer = FeedBack(folder_path,no_of_objects)
            self.feedback_viewer.show()
        else:
            self.control_widget.setVisible(True)
            self.status_label.setText("Edit all objects, select the output format, and press 'Create Assert' (use '.png' for best transparency).<br>"
                                        "<span style='color:red;'>Please select a valid output folder!!!!!</span>")
